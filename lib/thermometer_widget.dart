...
class ThermometerWidgetPainter extends CustomPainter {
  ...
  ThermometerWidgetPainter(
     { required this.borderColor,
      required his.innerColor,
      required his.indicatorColor,
      required his.textColor,
      required his.stemWidth,
      required his.temperature,
      required his.indicatorBulbWidth,
      required his.indicatorStemWidth}) {
    borderPaint = Paint()
      ..color = borderColor
  ...
  void drawThermometerShape(Canvas canvas, Size size) {
    canvas.drawRRect(RRect.fromRectAndRadius(Rect.fromLTWH(inset, 0.0, stemWidth, size.height), const Radius.circular(10.0)), borderPaint);

    canvas.drawCircle(Offset(size.width / 2, size.height - bulbRadius), bulbRadius, borderPaint);

    canvas.drawRRect(RRect.fromRectAndRadius(Rect.fromLTWH(inset, 0.0, stemWidth, size.height), const Radius.circular(10.0)), innerPaint);

    canvas.drawCircle(Offset(size.width / 2, size.height - bulbRadius), bulbRadius, innerPaint);
  }

  void drawIndicatorAndIndicatorText(Canvas canvas, Size size) {
    canvas.drawCircle(Offset(size.width / 2, size.height - bulbRadius), indicatorBulbWidth / 2, indicatorPaint);
    ...
    canvas.drawLine(Offset.zero, Offset(0.0, -(minTempHeight + tempHeight)), indicatorPaint);

    textStyle = TextStyle(color: textColor, fontFamily: 'Times New Roman', fontWeight: FontWeight.bold, fontSize: indicatorBulbWidth / 2);
  ...
  void drawGraduations(Canvas canvas, Size size) {
    ...
    int markPoint = 0;
    for (int i = 1; i <= 5; i++) {
      canvas.drawLine(Offset.zero, const Offset(graduationTickLength, 0.0), borderPaint);
      canvas.save();
      canvas.translate(graduationTickLength, 0.0);
      textStyle = TextStyle(color: borderColor, fontFamily: 'Calibri', fontSize: indicatorBulbWidth / 3);
      textPainter.text = TextSpan(
        text: '$markPoint°',
        style: textStyle,
      );
